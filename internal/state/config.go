...
func ReadConfig(log *log2.Log, fn string) *Config {
	...
	cc.readConfig(fn)
	gb := hcl.MergeBodies(cc.bodies)
	diag := gohcl.DecodeBody(gb, nil, &c)
	if diag.HasErrors() {
		for i := range diag {
			log.Errorf("decode config error(%s)", diag[i])
		}
	}

	// A2 := hclsyntax.Body{}
	// A2.Blocks = &file.Nav.Blocks[]

	// aaa := file.Body
	// fmt.Printf("\033[41m %v \033[0m\n", aaa)
	// aa, dd := file.Body.Content(includeFile)
	// aa := hb.Content()
	// fmt.Printf("\033[41m %v \033[0m\n", aa)
